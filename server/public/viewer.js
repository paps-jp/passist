...
(() => {
  ...
  try { statusEl.textContent = '📐 viewer v43 loaded'; statusEl.classList.remove('hidden'); } catch {}
  ...
  if (syncSizeBtn) {
    syncSizeBtn.onclick = () => {
      ...
      if (syncSizeOn) {
        ...
        // V-3: サイズ調整 OFF → ホスト側に restore イベントを送信し、 押下直前の元サイズに戻す
        let restored = false;
